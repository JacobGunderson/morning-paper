...
- `scripts/games/nyt.py` and `circle9.py`
...
Add an item to `config/games.yaml` with `id`, `title`, `provider`, and `url`. The daily mini crossword is an A2Z Puzzles iframe, and the regular daily crossword uses a local wrapper around the Solve the Crossword widget; both refresh automatically. Circle9 items can include an `embed_url`; the collector checks current framing headers and removes the embed if framing is denied.
